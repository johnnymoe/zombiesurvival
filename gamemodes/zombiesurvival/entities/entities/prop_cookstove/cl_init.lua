...
function ENT:Draw()
	...
	cam.Start3D2D(self:LocalToWorld(Vector(1, 0, self:OBBMaxs().z)), self:GetAngles(), 0.05)
		...
		draw.RoundedBox(64, w * -0.5, h * -0.30, w, h, color_black_alpha120)

		draw.SimpleText(translate.Get("cook_stove"), "ZS3D2DFont2", 0, 0, COLOR_GRAY, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		if MySelf:Team() == TEAM_HUMAN then
			...
			draw.SimpleText(translate.Get("start_cooking"), "ZS3D2DFont2", 0, 32, colFlash, TEXT_ALIGN_CENTER)
		end
