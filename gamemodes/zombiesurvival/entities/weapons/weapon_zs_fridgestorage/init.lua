...
function SWEP:SpawnGhost()
	...
	if owner and owner:IsValid() then
		owner:GiveStatus("ghost_fridgestorage")
	end
end
...
function SWEP:RemoveGhost()
	local owner = self:GetOwner()
	if owner and owner:IsValid() then
		owner:RemoveStatus("ghost_fridgestorage", false, true)
	end
end
...
function SWEP:PrimaryAttack()
	...
	local status = owner.status_ghost_fridgestorage
	...
	local ent = ents.Create("prop_fridgestorage")
	if ent:IsValid() then
		ent:SetPos(pos)
		ent:SetAngles(ang)
		ent:Spawn()
